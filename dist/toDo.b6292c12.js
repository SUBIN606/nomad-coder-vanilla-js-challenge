...
},{"./bundle-url":"node_modules/parcel-bundler/src/builtins/bundle-url.js"}],"src/styles/toDo.css":[function(require,module,exports) {
var reloadCSS = require('_css_loader');

module.hot.dispose(reloadCSS);
module.hot.accept(reloadCSS);
},{"_css_loader":"node_modules/parcel-bundler/src/builtins/css-loader.js"}],"src/js/toDo.js":[function(require,module,exports) {
"use strict";
...
var pendingList = [],
    finishedList = []; // localStorage에 저장된 toDo 불러오기

var loadTask = function loadTask() {
  var pendingTasks = JSON.parse(localStorage.getItem("pending"));
  var finishedTasks = JSON.parse(localStorage.getItem("finished"));
...
var saveTask = function saveTask(toDos) {
  // localStorage에서는 JS의 object를 그대로 인식하지 못함. String값으로 넣어줘야 함.
  toDos === pendingList ? localStorage.setItem("pending", JSON.stringify(toDos)) : localStorage.setItem("finished", JSON.stringify(toDos));
};
...
var deleteTask = function deleteTask(e) {
  ...
  if (li.parentNode === pendingArea) {
    var afterDelete = pendingList.filter(function (toDo) {
    ...
    var _afterDelete = finishedList.filter(function (toDo) {
      return toDo.id !== parseInt(li.id);
    });

    finishedList = _afterDelete;
...
var finishTask = function finishTask(e) {
  var li = e.target.parentNode;
  var finished = pendingList.find(function (toDo) {
    return toDo.id === parseInt(li.id);
  });
  finishedList.push(finished);
  saveTask(finishedList);
  paintTask(finishedArea, finished.id, finished.text);
  var afterFinish = pendingList.filter(function (toDo) {
    return toDo.id !== parseInt(li.id);
  });
...
var cancleTask = function cancleTask(e) {
  var li = e.target.parentNode;
  var cancled = finishedList.find(function (toDo) {
    return toDo.id === parseInt(li.id);
  });
  pendingList.push(cancled);
  saveTask(pendingList);
  paintTask(pendingArea, cancled.id, cancled.text);
  var afterCancle = finishedList.filter(function (toDo) {
    return toDo.id !== parseInt(li.id);
  });
...
var paintTask = function paintTask(area, newId, text) {
...
init();
},{"../styles/toDo.css":"src/styles/toDo.css"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","src/js/toDo.js"], null)
//# sourceMappingURL=/toDo.b6292c12.js.map